...
이 문서는 MiniX/kind Karmada 실험 00~29의 결과를 바탕으로, 실제 ScaleX-POD에서 ArgoCD, Karmada, Kueue를 어떤 역할로 나눠 사용할지 정리한다.
...
```text
kubernetes/karmada/runbooks/argocd-prune-safety.md
...
```text
kubernetes/karmada/runbooks/new-cluster-label-impact-checklist.md
```
...
```text
kubernetes/karmada/runbooks/kueue-observability.md
```
...
3. Karmada scheduler-estimator 설치형 capacity-aware scheduling
...
B. Kueue controller 장애/복구 실험
C. Kueue quota 변경 실험
D. ArgoCD AppProject migration 실험
E. scheduler-estimator 설치형 실험
F. 실제 ScaleX-POD 이전 checklist 작성
```
...
2. Kueue controller 장애/복구
3. Kueue quota 변경
4. ArgoCD AppProject migration
5. 실제 ScaleX-POD 이전 checklist
```
