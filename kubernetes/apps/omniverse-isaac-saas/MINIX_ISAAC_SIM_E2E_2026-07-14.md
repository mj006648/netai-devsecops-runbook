...
Read-only portal: http://10.34.48.222
...
### 7.2 최종 Harbor artifact

```text
10.34.48.223/omniverse/isaac-sim:6.0-netai-f2606b4-r3
sha256:5be32513c96b71a0b7224c87bd59b119f2e52b4461dd950dce14518acdcc0049
11,004,742,933 bytes compressed
```
...
실행 중 발견한 entrypoint 문제는 containerd에 이미 내려받은 11GB 이미지를 base로 사용해 작은 patch layer만 r1, r2, r3로 다시 push했다.
...
다만 OmniHub helper가 root container 환경에서 config file을 만들지 못하고 재연결하는 warning이 반복됐다.
...
사용자가 WebRTC client로 직접 확인할 수 있도록 최종 인스턴스를 삭제하지 않고 남겨뒀다.

```text
isaac-minix-e2e Deployment
isaac-minix-e2e Pod
isaac-minix-e2e-gpu ResourceClaim
isaac-minix-e2e-stream Service
Stream IP 10.34.48.224
```

내부 mutation portal도 현재 남아 있다.

```text
isaac-twinx-e2e Deployment
isaac-twinx-e2e ClusterIP Service
```
...
사용자 WebRTC 확인이 끝난 뒤 내부 API로 삭제한다.
...
```bash
kubectl -n omniverse get deploy,svc,resourceclaim | grep minix-e2e
...
Stream Service 없음
ResourceClaim 없음
포털 GPU status가 Available
Isaac Sim GPU process 없음
```

이번 실행 시점에는 사용자 접속을 위해 인스턴스를 유지했으므로, 이 E2E instance의 삭제와 반환 증거는 아직 기록하지 않았다.
...
2. OmniHub warning은 비차단이지만 Nucleus read/write 시험 전에 원인을 정리해야 한다.
3. 외부 포털은 아직 Keycloak과 연결하지 않았으므로 create/delete가 비활성화돼 있다.
4. 인스턴스 삭제와 DRA GPU 반환은 사용자 WebRTC 확인 후 수행한다.
5. Extension 9개는 등록만 했고 모두 자동 enable하지 않았다.
6. SmartX `eecs-k8s` chart와 `c-k8s`/`twinx-k8s` preset 반영은 아직 하지 않았다.
...
MiniX Kubernetes 1.34.3에서 private Harbor의 고정 Isaac Sim 6.0 이미지, NVIDIA DRA RTX 3090 정확 선택, Nucleus runtime config, 인스턴스별 LoadBalancer와 WebRTC Stream IP, 사용자 표시까지 실제 `Running` 상태로 검증했고, 다음 단계는 사용자 접속 후 delete/GPU 반환 증거를 추가한 다음 SmartX eecs-k8s chart와 cluster preset으로 이관하는 것이다.
