...
최신 Nucleus path 보정 Isaac Sim 이미지:

```text
repository: 10.34.48.223/omniverse/isaac-sim
tag: 6.0-netai-f2606b4-r4
digest: sha256:70399e7db9883341bae8539c8485ac5b17702cdb3a15ea218c687ccc1780c50f
source revision: c171142a33d6e5fd2daee018338e4d7adde99168
```

현재 사용자 WebRTC/delete 시험을 위해 보존한 `minix-e2e` instance는 r4 이전에 생성됐으므로 r3 digest를 계속 사용한다. 새 instance부터 r4 digest를 사용한다.
...
running minix-e2e digest: sha256:5be32513c96b71a0b7224c87bd59b119f2e52b4461dd950dce14518acdcc0049
```
...
기존 `minix-e2e` Deployment는 사용자의 WebRTC/delete 시험을 위해 r3 digest로 유지했다. 포털 설정 변경은 이미 존재하는 Deployment를 교체하지 않으므로, 삭제 후 새로 생성하는 instance부터 r4를 사용한다.
...
MiniX Kubernetes 1.34.3에서 private Harbor의 고정 Isaac Sim 6.0 이미지, NVIDIA DRA RTX 3090 정확 선택, Nucleus runtime config, 인스턴스별 LoadBalancer와 WebRTC Stream IP, 사용자 표시까지 실제 `Running` 상태로 검증했고, Nucleus path 보정 r4의 Harbor push/runtime pull과 개인 SmartX chart/preset 반영도 완료했으며, 다음 단계는 delete/GPU 반환과 r4 Nucleus read/write를 사용자 GUI에서 확인한 뒤 실제 GPU 대상 cluster의 eecs-k8s/preset Argo CD 이관을 수행하는 것이다.
