...
SmartX feature graph end-to-end 검증은 완료했습니다. 현재 추가로 남은 것은 “member cluster 내부의 여러 worker node 분산 scheduling”입니다.
...
완료:
  ...
  - Karmada PropagationPolicy/OverridePolicy server dry-run 검증

남음:
  - 별도 multi-node kind cluster에서 kube-scheduler nodeSelector/affinity/taint 배치 검증
```
...
즉 DataX preset은 PostgreSQL을 직접 켜지 않고, SmartX 엔진이 feature graph로 의존 앱을 자동 생성합니다.
...
```text
kubectl --kubeconfig ~/.kube/karmada-apiserver.config apply --dry-run=server -k federation
  overridepolicy.policy.karmada.io/scalex-render-demo created/configured
```

`OverridePolicy`는 같은 render Deployment에 대해 다음 차이를 만듭니다.

```text
twinx: render-role=primary, SCALEX_RENDER_CLUSTER=twinx
edgex: render-role=secondary, SCALEX_RENDER_CLUSTER=edgex
```
