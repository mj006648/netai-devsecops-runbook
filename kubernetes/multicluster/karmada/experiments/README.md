...
8. scalex-k8s federation repo와 *-k8s cluster-local repo split
```
...
```text
1. 실제 scalex-k8s/twinx-k8s/datax-k8s/edgex-k8s repo 생성과 bootstrap
...
| 33 | [`2026-07-07-33-scalex-repo-split-poc.md`](./2026-07-07-33-scalex-repo-split-poc.md) | scalex-k8s federation + *-k8s cluster-local repo split | 성공 | 임시 repo 4개로 scalex-k8s는 Karmada 전파, twinx/datax/edgex-k8s는 cluster-local 리소스를 담당하는 구조와 단일 소유권 원칙을 검증 |
