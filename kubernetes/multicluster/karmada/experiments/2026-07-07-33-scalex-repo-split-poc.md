# 실험 33. ScaleX repo split PoC: `scalex-k8s` federation + `*-k8s` cluster-local
...
```text
1. scalex-k8s는 멀티클러스터에 전파할 resource + Karmada policy repo로 둘 수 있는가?
...
```text
scalex-k8s = 어떤 리소스를 어느 클러스터들에 전파할 것인가?
```
...
```text
1. 멀티클러스터 전파 리소스는 scalex-k8s만 관리한다.
...
```text
/tmp/scalex-temp-repos/scalex-k8s
...
### 2. `scalex-k8s` namespace phase 적용
...
kubectl --kubeconfig ~/.kube/karmada-apiserver.config \
  apply -f /tmp/scalex-temp-repos/scalex-k8s/federation/00-namespaces/namespaces.yaml
```
...
### 3. `scalex-k8s` policy/app dry-run

```bash
kubectl --kubeconfig ~/.kube/karmada-apiserver.config apply --dry-run=server \
  -f /tmp/scalex-temp-repos/scalex-k8s/federation/00-namespaces/namespace-policies.yaml
kubectl --kubeconfig ~/.kube/karmada-apiserver.config apply --dry-run=server \
  -f /tmp/scalex-temp-repos/scalex-k8s/federation/10-apps/render-demo/app.yaml
kubectl --kubeconfig ~/.kube/karmada-apiserver.config apply --dry-run=server \
  -f /tmp/scalex-temp-repos/scalex-k8s/federation/10-apps/data-demo/app.yaml
```
...
이는 `scalex-k8s`가 멀티클러스터 placement repo로 동작할 수 있음을 보여준다.
...
```text
1. scalex-k8s는 멀티클러스터 전파 resource + Karmada policy repo로 동작 가능하다.
