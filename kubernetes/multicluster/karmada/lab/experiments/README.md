...
실험 00~33은 ScaleX-POD에서 Karmada를 실제 운영 구조로 사용할 수 있는지 판단하기 위한 기능 검증 세트다.
...
8. TowerX 단일 ArgoCD 기준 towerx-k8s/scalex-federation/*-k8s repo split
```

ScaleX-POD 관점의 최종 구조:

```text
ArgoCD: TowerX 단일 GitOps/source-of-truth sync
Karmada: scalex-federation resource의 ScaleX-POD member cluster placement/propagation/rebalance
Kueue: 필요 시 DataX/TwinX 내부 Job admission/quota/backpressure
```

따라서 다음 단계는 다음 순서로 진행한다.

```text
1. 실제 towerx-k8s/scalex-federation/twinx-k8s/datax-k8s/edgex-k8s repo 생성과 TowerX 단일 ArgoCD bootstrap
...
| 33 | [`2026-07-07-33-scalex-repo-split-poc.md`](./2026-07-07-33-scalex-repo-split-poc.md) | scalex-federation federation + *-k8s cluster-local repo split | 성공 | 임시 repo로 scalex-federation는 Karmada 전파, twinx/datax/edgex-k8s는 cluster-local 리소스를 담당하는 구조와 단일 소유권 원칙을 검증. 최종 운영은 TowerX 단일 ArgoCD가 각 repo/destination을 관리 |
...
- [ScaleX Karmada 운영 계획](../../README.md)
...
```text
ScaleX-POD 설계 판단에 필요한 핵심 기능 검증은 00~33에서 완료됐다.
