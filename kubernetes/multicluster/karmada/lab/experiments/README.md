...
8. TowerX 단일 ArgoCD 기준 tower-k8s/scalex-k8s/*-k8s repo split
```
...
Karmada: scalex-k8s resource의 ScaleX-POD member cluster placement/propagation/rebalance
...
```text
1. 실제 tower-k8s/scalex-k8s/twinx-k8s/datax-k8s/edgex-k8s repo 생성과 TowerX 단일 ArgoCD bootstrap
...
| 33 | [`2026-07-07-33-scalex-repo-split-poc.md`](./2026-07-07-33-scalex-repo-split-poc.md) | scalex-k8s federation + *-k8s cluster-local repo split | 성공 | 임시 repo로 scalex-k8s는 Karmada 전파, twinx/datax/edgex-k8s는 cluster-local 리소스를 담당하는 구조와 단일 소유권 원칙을 검증. 최종 운영은 TowerX 단일 ArgoCD가 각 repo/destination을 관리 |
