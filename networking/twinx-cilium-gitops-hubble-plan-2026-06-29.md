...
- merge 후 확인 결과 live `argocd` namespace에는 아직 `Application/cilium`이 없고, 기존 Cilium Helm release는 계속 `1.17.3`으로 동작 중이다.
...
- TwinX-Ops PR #219로 Cilium values와 app scaffold를 추가했다.
- TwinX-Ops PR #220으로 Cilium child app이 manual sync로 렌더링될 수 있게 했다.
- TwinX-Ops PR #221로 Cilium app을 enabled로 등록했다.
- Cilium GitOps 전환과 Hubble 활성화는 실제 적용하지 않았다.
- ArgoCD sync는 하지 않았다.

## 다음 작업

1. 작업 전 상태를 다시 보고하고 승인을 받은 뒤 `twinx-infra-root-app`만 수동 sync한다.
   - 목표는 live에 manual child `Application/cilium`을 생성하는 것이다.
   - 이 단계에서는 child `Application/cilium` sync, Cilium Helm apply, Hubble enable을 하지 않는다.
2. 생성된 child `Application/cilium`이 manual sync로 렌더링됐는지 확인한다.
   - `automated` syncPolicy가 없어야 한다.
3. child `Application/cilium` diff를 확인하고, 현재 Helm values와 Git values의 기능 차이가 없는지 검토한다.
4. edgebox3/4 전원 장애는 별도 하드웨어 이슈로 추적하고, Cilium 적용 단계에서는 known exception으로 남긴다.
5. 실제 Cilium ownership 전환 전에는 현재 상태, diff, rollback 계획을 다시 보고하고 승인을 받는다.
6. 별도 maintenance window에서 Cilium ownership 전환만 수행한다.
7. ownership 전환 안정화 후 Hubble relay와 metrics를 별도 커밋/승인으로 활성화한다.
8. Cilium version upgrade는 Hubble과 분리해 ArgoCD chart revision 변경으로 단계적으로 진행한다.
