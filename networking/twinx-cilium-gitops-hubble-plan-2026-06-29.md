...
5. **edgebox3/4 전원 문제와 control-plane 정리가 끝나기 전에는 적용하지 않는다.**
   - 불안정한 노드가 있는 상태에서 CNI 변경을 하면 원인 분리가 어려워진다.
...
목표는 배포가 아니라 Git에 구조와 values를 먼저 넣는 것이다.
...
1. edgebox3/4 전원 문제와 Kubernetes 상태를 먼저 안정화한다.
2. control-plane 1대 운영 전환 여부를 별도 작업으로 끝낸다.
3. TwinX-Ops에 `enabled: false` 상태의 Cilium app과 values를 추가한다.
4. 현재 Helm values와 Git values의 기능 차이가 없는지 검토한다.
5. 별도 maintenance window에서 Cilium ownership 전환만 수행한다.
6. 안정화 후 Hubble relay와 metrics를 별도 단계로 활성화한다.
