...
1. child `Application/cilium` diff를 확인하고, 현재 Helm values와 Git values의 기능 차이가 없는지 검토한다.
2. edgebox3/4 전원 장애는 별도 하드웨어 이슈로 추적하고, Cilium 적용 단계에서는 known exception으로 남긴다.
3. 실제 Cilium ownership 전환 전에는 현재 상태, diff, rollback 계획을 다시 보고하고 승인을 받는다.
4. 별도 maintenance window에서 Cilium ownership 전환만 수행한다.
